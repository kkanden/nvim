...
local function create_tinymist_command(command_name)
    ...
    local function run_tinymist_command()
        ...
        local client =
            vim.lsp.get_clients({ name = "tinymist", buffer = bufnr })[1]
        if not client then
            return vim.notify(
        ...
        local title_str = export_type and ("Export " .. cmd_display)
            or cmd_display
        ---@type lsp.Handler
        local function handler(err, res)
            ...
            vim.notify(
                export_type and res or vim.inspect(res),
                vim.log.levels.INFO
            )
        end
        if vim.fn.has("nvim-0.11") == 1 then
            ...
            return vim.notify(
                "Tinymist commands require Neovim 0.11+",
                vim.log.levels.WARN
            )
        end
    end
    -- Construct a readable command name/desc
    local cmd_name = export_type and ("Export" .. cmd_display)
    ...
    on_attach = function(_, bufnr)
        for _, command in ipairs({
            ...
            local cmd_func, cmd_name, cmd_desc =
                create_tinymist_command(command)
